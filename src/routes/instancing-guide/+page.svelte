...
<div class="guide-container">
	...
	<p>
		為了讓 Web 3D 專案能達到最佳效能，我們利用 <strong>GPU Instancing</strong> 技術來一次性繪製大量重複的物件。請在製作與輸出模型時，遵守以下規範。
	</p>
	...
	<div class="card">
		...
		<p>
			Instancing 的原理是：只載入一次模型幾何資料，然後告訴 GPU
			它出現在世界的哪些位置、旋轉角度與縮放大小。
		</p>
	</div>
	...
	<div class="card bad">
		...
		<ul>
			...
			<li>
				<strong>後果：</strong> 雖然兩個物件看起來一樣，但它們的頂點座標變得不同了，系統將無法視為同一個物件。
			</li>
		</ul>
	</div>

	<p>
		<strong>正確流程：</strong> 讓 Mesh 數據保持原始狀態（例如位於原點 (0,0,0)），所有的位置移動與旋轉，請保留在
		Object (Node) 層級上。
	</p>

	<h2>3. 材質一致性</h2>
	<p>
		所有實例物件必須共用同一個<strong>材質球 (Material)</strong
		>。如果兩個一樣的螺絲用了不同的材質，它們就無法被合併批次繪製。
	</p>

	<h2>4. 自我檢核 (Self-Check)</h2>
	<p>在輸出 GLB 之前，請在軟體的大綱視圖 (Outliner) 中檢查：</p>
	<p>
		是不是有<strong>多個物件 (Objects)</strong> 指向<strong>同一個網格數據 (Mesh Data)</strong>？
	</p>
	<ul>
		<li>✅ (Blender) Object A 和 Object B 的 Mesh 欄位都是 <code>Screw_Mesh</code>。</li>
		<li>
			❌ (Blender) Object A 是 <code>Screw_Mesh</code>，但 Object B 是 <code>Screw_Mesh.001</code>。
		</li>
	</ul>
</div>
